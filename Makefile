...
fix-composer: ## Fix Composer issues (git ownership, install dependencies)
	...
	docker compose exec --user www-data php-fpm-traditional git config --global --add safe.directory /var/www/html
	docker compose exec --user www-data php-fpm-traditional composer install --optimize-autoloader --no-scripts
	@echo "Composer issues fixed!"

clear-cache: ## Clear Symfony cache manually (without console command)
	@echo "Clearing Symfony cache..."
	docker compose exec --user www-data php-fpm-traditional rm -rf var/cache/*
	@echo "Cache cleared!"

warm-cache: ## Clear and warm Symfony cache (requires rate limiter)
	@echo "Clearing and warming Symfony cache..."
	docker compose exec --user www-data php-fpm-traditional rm -rf var/cache/*
	docker compose exec --user www-data php-fpm-traditional php bin/console cache:clear --no-warmup
...
test: ## Run PHPUnit tests
	@echo "Running tests..."
	docker compose exec --user www-data php-fpm-traditional vendor/bin/phpunit
...
install: ## Install Composer dependencies (run inside container)
	...
	docker compose exec --user www-data php-fpm-traditional composer install --optimize-autoloader
	@echo "Dependencies installed successfully!"

composer-update: ## Update Composer dependencies and create lock file
	@echo "Updating Composer dependencies..."
	docker compose exec --user www-data php-fpm-traditional composer update --lock
...
install-package: ## Install a Composer package (usage: make install-package package=symfony/rate-limiter)
	...
	USER_ID=$(shell id -u) GROUP_ID=$(shell id -g) docker compose exec --user www-data php-fpm-traditional composer require $(package) --no-scripts
...
cs-fix: ## Fix code style with PHP-CS-Fixer
	...
	USER_ID=$(shell id -u) GROUP_ID=$(shell id -g) docker compose exec --user www-data php-fpm-traditional vendor/bin/php-cs-fixer fix --config=.php-cs-fixer.php -v
	@echo "Code style fixed!"

phpstan: ## Run PHPStan static analysis
	@echo "Running PHPStan static analysis..."
	USER_ID=$(shell id -u) GROUP_ID=$(shell id -g) docker compose exec php-fpm-traditional sh -c 'XDEBUG_MODE=off  vendor/bin/phpstan analyse --configuration=phpstan.neon'
...
migrate: ## Run database migrations
	docker compose exec --user www-data php-fpm-traditional bin/console doctrine:migrations:migrate --no-interaction

db-create: ## Create database
	docker compose exec --user www-data php-fpm-traditional bin/console doctrine:database:create --if-not-exists

db-reset: ## Reset database (drop, create, migrate)
	docker compose exec --user www-data php-fpm-traditional bin/console doctrine:database:drop --force --if-exists
	docker compose exec --user www-data php-fpm-traditional bin/console doctrine:database:create
	docker compose exec --user www-data php-fpm-traditional bin/console doctrine:migrations:migrate --no-interaction

login:
	docker compose exec --user www-data -it php-fpm-traditional sh